...
use ruapc_rdma::{CompChannel, CompletionQueue, WRType, ibv_send_flags, ibv_wc};
use tokio::io::{Interest, unix::AsyncFd};

use crate::{Buffer, Error, ErrorKind, Message, Result, Socket, State};
        ...
        loop {
            ...
            tokio::select! {
                ...
                _ = self.pending_receiver.recv_many(&mut pending_sends, 256) => {
                    self.pending_sends.extend(pending_sends.drain(..).map(|msg| (msg.id, msg.buf)));
                },
