...
for count in args.connections:
    for pair in range(1, args.pairs + 1):
        for version in (['baseline', 'current'] if pair % 2 else ['current', 'baseline']):
            ...
            artifact = f'rpc-{count}-{version}-{pair}.json.gz'
            ...
            compressed = (args.output_dir / artifact).read_bytes()
            raw_bytes = gzip.decompress(compressed)
            raw = json.loads(raw_bytes)
            entry = {'connections': count, 'pair': pair, 'version': version, 'artifact': artifact, 'sha256': hashlib.sha256(compressed).hexdigest(), 'uncompressed_sha256': hashlib.sha256(raw_bytes).hexdigest()}
