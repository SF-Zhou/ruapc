...
const CASES: [(usize, usize); 1] = [(1024 * 1024, 10_000)];
static VERIFY_NS: std::sync::atomic::AtomicU64 = std::sync::atomic::AtomicU64::new(0);
static REMOTE_NS: std::sync::atomic::AtomicU64 = std::sync::atomic::AtomicU64::new(0);
...
impl RemoteMemoryBench for BenchService {
    ...
    async fn read_all(&self, ctx: &Context, req: &BenchRequest) -> Result<usize> {
        ...
        REMOTE_NS.fetch_add(timer.elapsed().as_nanos() as u64, Ordering::Relaxed);
...
fn verify(buffers: &[Buffer], expected: &[u8]) {
    ...
    VERIFY_NS.fetch_add(timer.elapsed().as_nanos() as u64, Ordering::Relaxed);
}
...
fn report(operation: &str, size: usize, iters: usize, elapsed: Duration) {
    ...
    println!(
        "{}",
        serde_json::json!({"phase":"diagnostic", "operation":operation, "size":size, "iterations":iters, "wall_us_per_op":us_per_op, "verify_us_per_op":VERIFY_NS.load(Ordering::Relaxed) as f64 / iters as f64 / 1000.0, "remote_await_us_per_op":REMOTE_NS.load(Ordering::Relaxed) as f64 / iters as f64 / 1000.0})
    );
    println!(
...
async fn bench_read(
    ...
    REMOTE_NS.store(0, Ordering::Relaxed);
...
async fn run(options: BenchOptions) {
    ...
    for transport in [
        ...
        for (case, &(size, iters)) in CASES.iter().enumerate() {
            ...
            match std::env::var("RUAPC_DIAG_DIRECTION").unwrap().as_str() {
                "read" => bench_read(&ctx, &request, expected, iters, options.warmup_iters).await,
                "write" => bench_write(&ctx, &request, expected, iters, options.warmup_iters).await,
                _ => panic!("RUAPC_DIAG_DIRECTION must be read or write"),
            }
        }
    }
    server.stop();
