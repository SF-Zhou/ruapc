...
/// Create a 64-byte `TestBuffer` registered with
...
pub fn test_buffer_with(
    ...
    let mr =
        unsafe { MemoryRegion::register(pd, data.as_ptr() as *mut _, data.len(), access.0 as _) }
            .unwrap();
    TestBuffer {
        _mr: mr,
        _data: data,
...
mod tests {
    ...
    use crate::test_utils::{open_device, test_buffer};
    use crate::*;

    /// Drop resources in the reverse of the natural creation order.
    /// If Arc ownership is wrong, this would cause a use-after-free / panic.
    #[test]
    fn test_drop_order_reverse() {
        ...
        let qp: QueuePair<crate::test_utils::TestBuffer> =
            QueuePair::create(&pd, &cq, &cq, &mut init_attr).unwrap();
