...
/// memory is freed.
///
/// A `Memory` can be created with no registrations via
/// [`new_unregistered`](Self::new_unregistered), then registered on
/// devices one at a time via [`Device::register`]. If a device's
/// registration fails, the `Memory` still holds registrations from
/// previously successful devices, ensuring correct cleanup on drop.
pub struct Memory<R: Registration> {
    aligned_memory: AlignedMemory,
    registrations: Vec<R>,
}

impl<R: Registration> Memory<R> {
    /// Creates a new `Memory` by allocating aligned memory and registering
    /// it on every device in `devices`.
    ///
    /// If any device registration fails, the `Memory` is dropped, which
    /// unregisters all previously successful registrations.
    ...
    /// Creates a new `Memory` with no registrations.
    ///
...
impl<R: Registration> Drop for Memory<R> {
    fn drop(&mut self) {
...
impl<R: Registration> std::fmt::Debug for Memory<R> {
