...
- **Logical spaces**: both sides of a transfer are *logical contiguous spaces* — ordered buffer/region lists, each segment contributing its logical `len()`. Clients attach a **read space** (`with_read_buffers(&[Buffer])`, borrowed) and/or a **write space** (`with_write_buffers(Vec<Buffer>)`, ownership moved & pinned) to a request; the regions travel in `MsgMeta.read_regions` / `write_regions`.
- **Vectored ops**: servers issue `CopyOp { src_offset, dst_offset, len }` batches via `Context::remote_read/remote_write` (`*_all` convenience for 1:1). Validation on both ends (bounds, overflow, `MAX_COPY_OPS`/`MAX_REGIONS`, non-overlapping dst) → `InvalidCopyOp`. RDMA fragments ops at remote-region boundaries into RDMA READ WRs with local SG lists (`core/scatter.rs`); TCP/WS/HTTP fall back to reverse RPCs (`MemoryService/read`/`push`) with inline data.
- **Write = client-side READ**: no RDMA WRITE verb; `remote_write` sends a reverse `MemoryService/pull` advertising the server's source buffers as read regions, and the *client* RDMA-READs into its pinned buffers. Client buffers live in an `Arc<WriteTarget>` on the waiter entry — handlers clone it while writing, so a timed-out request can never hand memory back to the pool while the NIC may still DMA into it. All attached write buffers return through `Result<WithBuffers<T>, E>` (witness: `SentBuffers` from a completed `remote_write` or `ctx.sent_nothing()`).
- **Read-batch machinery**: one `ReadBatch` per op batch (WRID→`Arc<ReadBatch>` in `rdma_completions`). In-flight READ WRs are bounded by a *per-local-NIC* semaphore (`rdma.max_inflight_read_wrs`, default 32, shared by all connections on the device across server `remote_read` and client `pull` — the read congestion control), plus a per-connection SQ-overflow guard (`qp.max_send_wr / 2`, not a policy knob). `max_rd_atomic`/`max_dest_rd_atomic` negotiated via `Endpoint.rd_atomic_cap` (min of device caps, ≤16). Post-read `is_message_waiting` liveness check guards the server-read path (one-sided reads are invisible to the client).
- **READ timeout**: `rdma.read_timeout_ms` (default 10s, 0 disables), enforced by the poll thread's periodic sweep (no per-op timers). Timeout fails the waiter with `RdmaReadTimeout` and moves the QP to ERR; the batch's memory hold is only released after *every* (flush) completion arrives — never force-recycled. Connections with outstanding read batches are not torn down (`ready_to_remove`), and poller shutdown explicitly fails pending batches.
...
- **Client retries**: `Client.max_retries` (default 2) retries only pre-wire failures (acquire/send); the waiter entry is allocated *after* connect so slow connection setup doesn't consume the response budget. `Context::with_addrs` / `with_addr_set` give round-robin multi-address (equivalent addresses of *one* server, e.g. one per NIC) with health-aware failover: each request walks an attempt plan with quarantined addresses last, address-indicting failures (connect/send, or the connection dying mid-flight with `ConnectionClosed`) quarantine the address for the `AddrSet` cooldown (default 30s; *soft* — when everything is down requests proceed anyway), and an address that works again is reinstated immediately. Health state lives in the `Arc<AddrSet>`; share it across contexts via `Context::with_addr_set`.
- **Connect gating**: outgoing TCP/WS/HTTP connects are bounded by `SocketPoolConfig.connect_timeout_ms` (default 5s, `0` disables; covers TCP connect *plus* protocol handshake) so a blackholed peer (downed NIC) fails over in bounded time instead of the OS SYN-retry limit. Connects are serialized *process-wide per address* across independent pools and transport variants (`ConnectGate`, RAII permit with pruned global lock map) — each pool's socket map is only locked for lookup/insert, never across a connect, so a stalled dial cannot block acquires for other peers. RDMA bootstrap TCP dials use the same gate; complete RDMA QP handshakes use a separate process-wide per-peer registry to avoid recursively locking the bootstrap address. Sockets are inserted into the map *before* their IO loops spawn, so identity-checked eviction always observes the entry.
