...
- **Completion evidence**: `CompletionQueue::poll_batch` lends non-cloneable CQ-issued tokens from private batch storage. `QueuePair::complete` checks the originating CQ, hardware QPN and CQ-owned `CompletionIdentity` (QPN plus immutable sequence floor) before recovering SEND/RECV buffers or settling READ ownership. Complete identity is `(CQ, QPN, WRType, sequence)`; WRIDs contain 2 type bits and 62 sequence bits, independent of CQ capacity. QPs use dense local sequences; SQ and RQ are independent, and SEND variants plus READ share SQ. After provider QP creation assigns a QPN, that QPN is leased in each associated CQ before posting; setup rollback and final teardown destroy the provider QP before releasing leases or DMA holds. Each CQ keeps only live QPN-to-floor entries and one `retired_floor`, giving O(peak live QPs) registry storage. New leases use that floor; retirement advances it to `max(retired_floor, SQnext, RQnext, first_sequence + 1)` and removes the entry under one mutex. Reusing the same `(CQ, QPN)` therefore starts above every old allocation, including failed posts. Unrelated live QPs retain their starting floors when the CQ watermark rises. Only setup, destruction and introspection take the registry lock; posting needs no per-WR global counter. Sequence allocation never wraps; a retired floor of `2^62` rejects new registrations on that CQ, with no reset when it becomes empty. Core pollers use a QPN hash map and reject older sequences before flow accounting. Each CQ drain handles at most 16 batches of 64 CQEs (1024 completions), returning to maintenance earlier on a short batch. CQEs schedule only touched connections for immediate maintenance; full housekeeping runs every 100 ms or on an undirected external wake and can scale with the map's retained peak capacity. Raw CQE metadata cannot authorize recovery. If the provider cannot destroy a QP, the process aborts before releasing memory still accessible to DMA. Core CQ admission uses `sum(R + W + A) + min(H, sum(K))` software completion credits, selects a fitting CQ shard before QP creation, and retains the reservation until QP destruction followed by a retirement snapshot and an actual empty CQ poll. Snapshots survive drain limits and short nonempty batches across poll-loop rounds; only an empty poll after the snapshot releases its budget. Identity retirement alone does not release this capacity budget. See `docs/wrid.md` for ownership, registration ordering and bounds, and `docs/qp-registry.md` for current validation and performance evidence.
- **READ timeout**: `rdma.remote_memory.read_timeout_ms` (default 10s, 0 disables), enforced by the poll thread's periodic sweep (no per-op timers). Timeout fails the waiter with `RdmaReadTimeout` and moves the QP to ERR; destination memory remains held through all posted success/error/flush completions or successful QP destruction. `ready_to_remove` waits for pending READs and all per-QP READ permits to return after admission closes. Fatal poller shutdown first closes READ admission on all incoming and live sockets and fails receivers, then releases its socket owners. Final socket destruction returns NIC permits for the QP's actual outstanding READs only after successful QP destruction; external socket owners delay recovery. Moving to ERR or failing waiters does not return posted READ permits or release DMA holds. Dropping or forgetting a request future cannot authorize early recovery; forgetting ownership can retain resources indefinitely. The server-read path still uses `_ruapc.memory/request_is_pending` to reject results from expired source requests, subject to the source-lifetime limitation above.
