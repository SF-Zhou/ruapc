...
- **Static policy**: `rdma.device_filter` defines the allowed local NICs when a context is created. On the connecting client, `rdma.subnets` lists CIDRs; a path matches when both NIC addresses belong to any one configured subnet. `rdma.subnet_policy` controls whether such paths are preferred or required before link class and load. The server does not use local subnet configuration for matching. Applications needing different policies create independent contexts.
