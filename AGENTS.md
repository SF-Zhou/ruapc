...
- `rdma/` — RDMA framing, connection/path policy and CQ polling; `rdma_socket/read.rs` owns READ execution, `read/batch.rs` owns completion lifetime, `poller/flow.rs` owns credits

See `DESIGN.md` for ownership invariants and `docs/refactoring.md` for validation evidence.
